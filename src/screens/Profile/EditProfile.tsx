...
const EditProfile = () => {
  ...
  const [collaboratorUpdateDates, setCollaboratorUpdateDates] =
    useState<inputUpdateDates>({
      name: "",
      sex: "M",
      phone: "",
      email: "",
  ...
  const convertChildrenToObject = (
    childrenArray: { name: string; age: string }[]
  ) => {
      ...
      switch (response.status) {
        case 200:
          await AsyncStorage.setItem(
            "collaborator",
            JSON.stringify(response.collaborator.collaborator)
          );
    ...
    if (collaborator) {
      ...
      if (collaborator.children && typeof collaborator.children === "object") {
        ...
        const childrenArray = Object.keys(collaborator.children).map((key) => {
          const child = (collaborator.children as { [key: string]: ChildType })[
            key
          ]; // Faz um cast explícito
          return {
            name: child.name,
            ...
            <View className={`w-4/6 `}>
              ...
              >
                ...
                {collaborator && collaborator.update_at
                  ? `${Mask("dateFormatBrazil", collaborator.update_at)}`
                  : Mask("dateFormatBrazil", collaborator?.create_at ?? "")}
              </Text>
            </View>
          </View>
        </View>
        ...
        >
          ...
          <View className={`mt-2`} style={{ marginBottom: 15 }}>
            <View className={`mb-1`}>
              <View className={`flex-row items-end mb-2`}>
                <Image
                  source={IMAGES.transgenderDuotone}
                  style={[styles.icon]}
                />
                <Text className={`text-[#222222]`} style={FONTS.fontRegular}>
                  Qual seu sexo?
                </Text>
              </View>
          ...
          <View style={{ marginBottom: 15 }}>
            <View className={`mb-1`}>
              <View className={`flex-row items-end mb-2`}>
                <Image
                  source={IMAGES.WheelchairDuoTone}
                  style={[styles.icon]}
                />
                <Text className={`text-[#222222]`} style={FONTS.fontRegular}>
                  Você é PCD?
          ...
          <View>
            ...
            <View className={`px-5`} style={{ marginBottom: 15 }}>
              ...
              <View className={`mb-3 flex-row items-center justify-between`}>
                ...
                <View>
                  <Text
                    className={`text-[#222222] text-xs`}
                    style={FONTS.fontRegular}
                  >
                    Tem número?
                  </Text>
                  <ToggleStyle3
                    active={hasAddressNumber}
                    setActive={setHasAddressNumber}
                  />
                </View>
              </View>
