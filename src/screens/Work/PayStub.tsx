...
const PayStub = ({ jobConected, CPF }: Props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (jobConected) {
        try {
          const response = await CheckDocumentServices(jobConected.id, mes, ano, 'point');
  ...
  return (
    <View className="flex-1 p-4">
      ...
      />
      <Text className="text-xl font-bold">Informe o mês/ano, o tipo de holerite e clique em "Visualizar holerite":</Text>
      ...
      <TouchableOpacity className="bg-primary py-4 rounded" onPress={() => {}}>
        <Text className="text-white text-center text-xl font-bold">VISUALIZAR holerite</Text>
      </TouchableOpacity>
