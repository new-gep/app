...
const Point = () => {
  ...
  const fetchData = async () => {
    if (jobConected && jobConected.id && mes && year) {
      try {
        setIsFetching(true);
        const monthInEnglish = months.find((m) => m.label === mes)?.english;
        ...
        Alert.alert("Erro", "Não foi possível carregar os documentos. Verifique sua conexão e tente novamente.");
      } finally {
        setIsFetching(false);
      }
    }
            ...
            return (
              <View
                ...
                className={`mb-4 bg-white rounded-xl shadow-sm ${
                  !item.fileName.match(/^Service_Point_\d{4}_[A-Za-z]+_\d+$/) && "hidden"
                }`}
              >
                <TouchableOpacity
                ...
                >
                  ...
                  <View className="flex-1">
                    <Text className="text-lg font-semibold text-gray-800">
                      Ponto {months.find((m) => m.label === mes)?.label}{" "}
                      {year}
                    </Text>
                    <Text
                      className={`text-base ${
