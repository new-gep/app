...
const SingIn = ({ route, navigation }: SingInScreenProps) => {
  ...
  const handleFocus = useCallback(() => {
    const searchRegister = async () => {
      const newCollaborator = await AsyncStorage.getItem(
        "collaboratorCreateSuccess"
      );
      const newCollaboratorPassword = await AsyncStorage.getItem(
        "collaboratorCreateNewPasswordSuccess"
      );
      if (newCollaborator) {
          ...
          <View style={{}}>
            <View style={{ marginBottom: 30 }}>
              ...
              <Text style={[styles.title2, { color: colors.title }]}>
                Entre com facilidade e segurança. Estamos aqui para facilitar
                seu dia a dia, desde o primeiro acesso.
              </Text>
            </View>
