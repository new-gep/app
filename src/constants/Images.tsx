export const IMAGES = {
  user: require("../assets/images/user.png"),
  WheelchairDuoTone: require("../assets/images/icons/WheelchairDuoTone.png"),
  card: require("../assets/images/card.png"),
  card1: require("../assets/images/card1.png"),
  Vector: require("../assets/images/Vector.png"),
  cardbackground1: require("../assets/images/cardbackground1.png"),
  cardbackground2: require("../assets/images/cardbackground2.png"),
  appname: require("../assets/picture/logo/blackSimple.png"),
  appname2: require("../assets/picture/logo/yellowSimple.png"),
  appnamedark: require("../assets/picture/logo/yellowSimple.png"),
  headerlogo: require("../assets/images/headerlogo.png"),
  google2: require("../assets/images/icons/google2.png"),
  facebook: require("../assets/images/icons/facebook.png"),
  Home: require("../assets/images/icons/home.png"),
  Home4: require("../assets/images/icons/home4.png"),
  pasteWork: require("../assets/images/icons/pasteWork.png"),
  heart: require("../assets/images/icons/heart.png"),
  mycart: require("../assets/images/icons/shopping2.png"),
  Category: require("../assets/images/icons/document.png"),
  user2: require("../assets/images/icons/user3.png"),
  user3: require("../assets/images/icons/user4.png"),
  user1: require("../assets/images/icons/user5.png"),
  grid3: require("../assets/images/icons/grid3.png"),
  grid4: require("../assets/images/icons/grid4.png"),
  grid5: require("../assets/images/icons/grid5.png"),
  grid6: require("../assets/images/icons/grid6.png"),
  shose: require("../assets/images/icons/shose.png"),
  ball: require("../assets/images/icons/bell2.png"),
  ball2: require("../assets/images/icons/bell.png"),
  close: require("../assets/images/icons/close.png"),
  check5: require("../assets/images/icons/check.png"),
  check2: require("../assets/images/icons/check2.png"),
  check3: require("../assets/images/icons/check3.png"),
  check4: require("../assets/images/icons/check4.png"),
  delete: require("../assets/images/icons/delete.png"),
  Chat: require("../assets/images/icons/Chat.png"),
  Star: require("../assets/images/icons/Star.png"),
  Star4: require("../assets/images/icons/star4.png"),
  wallet: require("../assets/images/icons/wallet.png"),
  wallet2: require("../assets/images/icons/wallet2.png"),
  Location: require("../assets/images/icons/Location.png"),
  Filter: require("../assets/images/icons/Filter.png"),
  Notification: require("../assets/images/icons/Notification.png"),
  search: require("../assets/images/icons/search.png"),
  list: require("../assets/images/icons/list.png"),
  list2: require("../assets/images/icons/list2.png"),
  grid: require("../assets/images/icons/grid.png"),
  arrowup: require("../assets/images/icons/arrowup.png"),
  filter2: require("../assets/images/icons/filter2.png"),
  filter3: require("../assets/images/icons/filter3.png"),
  shop: require("../assets/images/icons/shop.png"),
  Bag: require("../assets/images/icons/Bag.png"),
  plus: require("../assets/images/icons/plus.png"),
  plusDuotone: require("../assets/images/icons/plusDuotone.png"),
  transgenderDuotone: require("../assets/images/icons/transgenderDuotone.png"),
  dollar: require("../assets/images/icons/dollar.png"),
  payment: require("../assets/images/icons/payment.png"),
  folder: require("../assets/images/icons/folder.png"),
  bank: require("../assets/images/icons/bank.png"),
  shieldcheck: require("../assets/images/icons/shieldcheck.png"),
  card2: require("../assets/images/icons/card2.png"),
  write: require("../assets/images/icons/write.png"),
  downaeeowsmall: require("../assets/images/icons/downaeeowsmall.png"),
  home: require("../assets/images/icons/home4.png"),
  producta: require("../assets/images/icons/producta.png"),
  components: require("../assets/images/icons/components.png"),
  star: require("../assets/images/icons/star2.png"),
  heart2: require("../assets/images/icons/heart2.png"),
  order: require("../assets/images/icons/order.png"),
  shopping: require("../assets/images/icons/shopping3.png"),
  chat: require("../assets/images/icons/chat2.png"),
  logout: require("../assets/images/icons/logout.png"),
  call: require("../assets/images/icons/call.png"),
  ring: require("../assets/picture/icons/rings.png"),
  ringDuotone: require("../assets/images/icons/ringDuotone.png"),
  children: require("../assets/picture/icons/children.png"),
  childrenDuotone: require("../assets/images/icons/childrenDuotone.png"),
  completeProfile1: require("../assets/picture/unique/unique2.png"),
  completeProfile2: require("../assets/picture/unique/unique3.png"),
  completeProfile3: require("../assets/picture/unique/unique4.png"),
  completeDocument1: require("../assets/picture/unique/unique5.png"),
  completeDocument2: require("../assets/picture/unique/unique6.png"),
  completeDocument3: require("../assets/picture/unique/unique7.png"),
  happy: require("../assets/images/icons/happy.png"),
  send: require("../assets/images/icons/send.png"),
  volume: require("../assets/images/icons/volume.png"),
  phone: require("../assets/images/icons/phone.png"),
  audio: require("../assets/images/icons/audio.png"),
  audiomute: require("../assets/images/icons/audiomute.png"),
  sun: require("../assets/images/icons/sun.png"),
  darkMode: require("../assets/images/icons/moon.png"),
  fire: require("../assets/images/icons/fire.png"),
  deliverytruck: require("../assets/images/icons/delivery-truck.png"),
  deliverytruck2: require("../assets/images/icons/delivery-truck2.png"),
  savemoney: require("../assets/images/icons/save-money.png"),
  technicalsupport: require("../assets/images/icons/technical-support.png"),
  timer: require("../assets/images/icons/timer.png"),
  offer: require("../assets/images/icons/offer.png"),
  gift: require("../assets/images/icons/gift1.png"),
  background: require("../assets/images/icons/background.png"),
  star7: require("../assets/images/icons/star7.png"),
  star8: require("../assets/images/icons/star8.png"),
  leftarrow: require("../assets/images/icons/leftarrow.png"),
  map: require("../assets/images/icons/map.png"),
  savecheck: require("../assets/images/icons/save-check.png"),
  translation: require("../assets/images/icons/translation.png"),
  transfer: require("../assets/images/icons/transfer.png"),
  shoppingbag: require("../assets/images/icons/shoppingbag.png"),
  mobilenotch: require("../assets/images/icons/mobile-notch.png"),
  marketplacestore: require("../assets/images/icons/marketplace-store.png"),
  laptopmobile: require("../assets/images/icons/laptop-mobile.png"),
  gamepad: require("../assets/images/icons/gamepad.png"),
  couch: require("../assets/images/icons/couch.png"),
  cameraretro: require("../assets/images/icons/camera-retro.png"),
  qrscan: require("../assets/images/icons/qr-scan.png"),
  airfreshener: require("../assets/images/icons/air-freshener.png"),
  fashion: require("../assets/images/icons/fashion.png"),
  email: require("../assets/images/icons/email.png"),
  email2: require("../assets/images/icons/email2.png"),
  lock: require("../assets/images/icons/lock.png"),
  bolt: require("../assets/images/icons/bolt.png"),
  biking: require("../assets/images/icons/biking.png"),
  tachometerfast: require("../assets/images/icons/tachometer-fast.png"),
  scale: require("../assets/images/icons/scale.png"),
  truckcheck: require("../assets/images/icons/truck-check.png"),
  Phoneduotone: require("../assets/images/icons/Phone_duotone.png"),
  Pinduotone: require("../assets/images/icons/Pin_duotone.png"),
  hamburger: require("../assets/images/icons/hamburger.png"),
  cup: require("../assets/images/icons/cup.png"),
  share: require("../assets/images/icons/share1.png"),
  information: require("../assets/images/icons/information.png"),
  cake: require("../assets/images/icons/cake.png"),
  welcome: require("../assets/images/onbording/welcome.png"),
  welcome2: require("../assets/images/onbording/welcome2.png"),

  setting_outline: require("../assets/picture/icons/settings_outline.png"),
  setting_filled: require("../assets/picture/icons/settings_filled.png"),
  home_outline: require("../assets/picture/icons/home_outline.png"),
  home_filled: require("../assets/picture/icons/home_filled.png"),
  work_outline: require("../assets/picture/icons/work_outline.png"),
  work_filled: require("../assets/picture/icons/work_filled.png"),
  save_outline: require("../assets/picture/icons/save_outline.png"),
  save_filled: require("../assets/picture/icons/save_filled.png"),
  heart_outline: require("../assets/picture/icons/heart.png"),
  wine_outline: require("../assets/picture/icons/wine.png"),
  pet_outline: require("../assets/picture/icons/pet.png"),
  smoke_outline: require("../assets/picture/icons/smoke.png"),
  pizza_outline: require("../assets/picture/icons/pizza.png"),
  school_outline: require("../assets/picture/icons/school.png"),
  chat_outline: require("../assets/picture/icons/chat.png"),
  book_outline: require("../assets/picture/icons/book.png"),
  document_outline: require("../assets/picture/icons/document.png"),
  eye_outline: require("../assets/picture/icons/eye.png"),
  robot_outline: require("../assets/picture/icons/robot.png"),
  upload_outline: require("../assets/picture/icons/upload.png"),
  cv_outline: require("../assets/picture/icons/cv.png"),
  signature_outline: require("../assets/picture/icons/signature.png"),
  pen_outline: require("../assets/picture/icons/pen.png"),
  key_outline: require("../assets/picture/icons/key.png"),
  star_outline: require("../assets/picture/icons/star.png"),
  user_outline: require("../assets/picture/icons/user.png"),
  help_outline: require("../assets/picture/icons/help.png"),
  support_outline: require("../assets/picture/icons/support.png"),
  logout_outline: require("../assets/picture/icons/logout.png"),
  exit_outline: require("../assets/picture/icons/exit.png"),
  clock_outline: require("../assets/picture/icons/clock.png"),
  savings_outline: require("../assets/picture/icons/savings.png"),
  handShake_outline: require("../assets/picture/icons/handShake.png"),
  homeWork_outline: require("../assets/picture/icons/homeWork.png"),
  location_outline: require("../assets/picture/icons/location.png"),
  image_outline: require("../assets/picture/icons/image.png"),
  imageLibrary_outline: require("../assets/picture/icons/imageLibrary.png"),
  wallet_outline: require("../assets/picture/icons/wallet.png"),
  volunter_outline: require("../assets/picture/icons/volunter.png"),
  bus_outline: require("../assets/picture/icons/bus.png"),
  delete_outline: require("../assets/picture/icons/delete.png"),
  house_outline: require("../assets/picture/icons/house.png"),
  car_outline: require("../assets/picture/icons/car.png"),
  styler_outline: require("../assets/picture/icons/styler.png"),
  health_outline: require("../assets/picture/icons/health.png"),
  celebration_outline: require("../assets/picture/icons/celebration.png"),
  computer_outline: require("../assets/picture/icons/computer.png"),
  tools_outline: require("../assets/picture/icons/tools.png"),
  build_outline: require("../assets/picture/icons/build.png"),
  selfCare_outline: require("../assets/picture/icons/selfCare.png"),
  deskPhone_outline: require("../assets/picture/icons/deskPhone.png"),
  kitchen_outline: require("../assets/picture/icons/kitchen.png"),
  radio_outline: require("../assets/picture/icons/radio.png"),
  book2_outline: require("../assets/picture/icons/book2.png"),
  theater_outline: require("../assets/picture/icons/theater.png"),
  soccer_outline: require("../assets/picture/icons/soccer.png"),
  code_outline: require("../assets/picture/icons/code.png"),
  carSell_outline: require("../assets/picture/icons/carSell.png"),
  carRepair_outline: require("../assets/picture/icons/carRepair.png"),
  glassBroken_outline: require("../assets/picture/icons/glassBroken.png"),
  roller_outline: require("../assets/picture/icons/roller.png"),
  power_outline: require("../assets/picture/icons/power.png"),
  balance_outline: require("../assets/picture/icons/balance.png"),
  camera_outline: require("../assets/picture/icons/camera.png"),
  trendingUp_outline: require("../assets/picture/icons/trendingUp.png"),
  paid_outline: require("../assets/picture/icons/paid.png"),
  stockMedia_outline: require("../assets/picture/icons/stockMedia.png"),
  animatedVideo_outline: require("../assets/picture/icons/animatedVideo.png"),
  folder_outline: require("../assets/picture/icons/folder.png"),
  social_outline: require("../assets/picture/icons/@.png"),
  money_outline: require("../assets/picture/icons/money.png"),
  receipt_outline: require("../assets/picture/icons/receipt.png"),
  family_outline: require("../assets/picture/icons/family.png"),
  skeleton_outline: require("../assets/picture/icons/skeleton.png"),
  personInjury_outline: require("../assets/picture/icons/personInjury.png"),
  vaccine_outline: require("../assets/picture/icons/vaccine.png"),
  brain_outline: require("../assets/picture/icons/brain.png"),
  gardenCart_outline: require("../assets/picture/icons/gardenCart.png"),
  boxRepair_outline: require("../assets/picture/icons/boxRepair.png"),
  engineering_outline: require("../assets/picture/icons/engineering.png"),
  machine_outline: require("../assets/picture/icons/machine.png"),
  groups_outline: require("../assets/picture/icons/groups.png"),
  chef_outline: require("../assets/picture/icons/chef.png"),
  note_outline: require("../assets/picture/icons/note.png"),
  add_outline: require("../assets/picture/icons/add.png"),
  shirt_outline: require("../assets/picture/icons/shirt.png"),
  makeup_outline: require("../assets/picture/icons/makeup.png"),
  box_outline: require("../assets/picture/icons/box.png"),
  touchApp_outline: require("../assets/picture/icons/touchApp.png"),
  campaign_outline: require("../assets/picture/icons/campaign.png"),
  subDirectory: require("../assets/picture/icons/subDirectory.png"),
  history_outline: require("../assets/picture/icons/history.png"),
  search_outline: require("../assets/picture/icons/search.png"),
  addCircle_outline: require("../assets/picture/icons/addCircle.png"),
  flash_outline: require("../assets/picture/icons/flash.png"),
  notification_outline: require("../assets/picture/icons/notification.png"),
  pageInfo_outline: require("../assets/picture/icons/pageInfo.png"),
  menu: require("../assets/picture/icons/menu.png"),
  menu_outline: require("../assets/picture/icons/menu_outline.png"),
  menu_filled: require("../assets/picture/icons/menu_filled.png"),
  heart_filled: require("../assets/picture/icons/heart_filled.png"),


  door: require("../assets/images/icons/door.png"),
  medical: require("../assets/images/icons/medical.png"),
  warning: require("../assets/images/icons/warning.png"),
  swap: require("../assets/images/icons/swap.png"),
  arrowDown: require("../assets/images/icons/arrowDown.png"),
  arrowRight: require("../assets/images/icons/arrowRight.png"),
  picture: require("../assets/images/icons/picture.png"),
  lockOpen: require("../assets/images/icons/lockOpen.png"),
  lockClose: require("../assets/images/icons/lockClose.png"),
  pcd: require("../assets/images/icons/pcd.png"),
  cakeDuotone: require("../assets/images/icons/cakeDuotone.png"),

  onborder5: require("../assets/picture/onboarding/1.png"),
  onborder4: require("../assets/picture/onboarding/2.png"),
  onborder3: require("../assets/picture/onboarding/3.png"),

  item1: require("../assets/images/item/pic1.png"),
  item2: require("../assets/images/item/pic2.png"),
  item3: require("../assets/images/item/pic3.png"),
  item11: require("../assets/images/item/pic4.png"),
  item12: require("../assets/images/item/pic5.png"),
  item13: require("../assets/images/item/pic6.png"),
  item14: require("../assets/images/item/pic7.png"),
  item15: require("../assets/images/item/pic8.png"),
  item16: require("../assets/images/item/pic9.png"),

  item4: require("../assets/images/item2/pic1.png"),
  item5: require("../assets/images/item2/pic2.png"),
  item6: require("../assets/images/item2/pic3.png"),

  brand1: require("../assets/images/brand/pic1.png"),
  brand2: require("../assets/images/brand/pic2.png"),
  brand3: require("../assets/images/brand/pic3.png"),
  brand4: require("../assets/images/brand/pic4.png"),
  brand5: require("../assets/images/brand/pic5.png"),
  brand6: require("../assets/images/brand/pic6.png"),
  brand7: require("../assets/images/brand/pic7.png"),
  brand8: require("../assets/images/brand/pic8.png"),
  brand9: require("../assets/images/brand/pic9.png"),
  brand10: require("../assets/images/brand/pic10.png"),
  brand11: require("../assets/images/brand/pic11.png"),
  brand12: require("../assets/images/brand/pic12.png"),
  brand13: require("../assets/images/brand/pic13.png"),
  brand14: require("../assets/images/brand/pic14.png"),
  brand15: require("../assets/images/brand/pic15.png"),
  brand16: require("../assets/images/brand/pic16.png"),
  brand17: require("../assets/images/brand/pic17.png"),

  small1: require("../assets/images/small/pic5.png"),
  small2: require("../assets/images/small/pic6.png"),
  small3: require("../assets/images/small/pic7.png"),
  small4: require("../assets/images/small/pic8.png"),
  small5: require("../assets/images/small/pic9.png"),
  small6: require("../assets/images/small/pic10.png"),
  small7: require("../assets/images/small/pic11.png"),

  flags1: require("../assets/images/flags/pic1.png"),
  flags2: require("../assets/images/flags/pic2.png"),
  flags3: require("../assets/images/flags/pic3.png"),
  flags4: require("../assets/images/flags/pic4.png"),
  flags5: require("../assets/images/flags/pic5.png"),
  flags6: require("../assets/images/flags/pic6.png"),
  flags7: require("../assets/images/flags/pic7.png"),
  flags8: require("../assets/images/flags/pic8.png"),
  flags9: require("../assets/images/flags/pic9.png"),
  flags10: require("../assets/images/flags/pic10.png"),
  flags11: require("../assets/images/flags/pic11.png"),
  flags12: require("../assets/images/flags/pic12.png"),
  flags13: require("../assets/images/flags/pic13.png"),
  flags14: require("../assets/images/flags/pic14.png"),

  unique0: require("../assets/picture/unique/unique0.png"),
  unique1: require("../assets/picture/unique/unique1.png"),
  unique8: require("../assets/picture/unique/unique9.png"),
  unique9: require("../assets/picture/unique/unique8.png"),
  unique10: require("../assets/picture/unique/unique10.png"),
  unique11: require("../assets/picture/unique/unique11.png"),
  unique12: require("../assets/picture/unique/unique12.png"),
  unique13: require("../assets/picture/unique/unique13.png"),
  unique14: require("../assets/picture/unique/unique14.png"),
  unique15: require("../assets/picture/unique/unique17.png"),
  unique16: require("../assets/picture/unique/unique16.png"),
  unique17: require("../assets/picture/unique/unique15.png"),
  unique18: require("../assets/picture/unique/unique18.png"),
  unique19: require("../assets/picture/unique/unique19.png"),
  unique20: require("../assets/picture/unique/unique20.png"),
  unique21: require("../assets/picture/unique/unique21.png"),
  unique22: require("../assets/picture/unique/unique22.png"),
  unique23: require("../assets/picture/unique/unique23.png"),
  unique24: require("../assets/picture/unique/unique24.png"),
  unique25: require("../assets/picture/unique/unique25.png"),
  unique26: require("../assets/picture/unique/unique26.png"),
  unique27: require("../assets/picture/unique/unique27.png"),
  unique28: require("../assets/picture/unique/unique28.png"),
  unique29: require("../assets/picture/unique/unique29.png"),

  gif0: require("../assets/picture/gif/unique0.gif"),
};
